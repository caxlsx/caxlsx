module Axlsx
  ...
  class DataValidation
    ...
    def to_xml_string(str = '')
      ...
      str << '>'
      str << ('<formula1>' << self.formula1 << '</formula1>') if @formula1 && valid_attributes.include?(:formula1)
      str << ('<formula2>' << self.formula2 << '</formula2>') if @formula2 && valid_attributes.include?(:formula2)
      str << '</dataValidation>'
    end
